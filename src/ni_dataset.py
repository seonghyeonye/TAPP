...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None, demo_num=None, **kwargs):
            ...
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if subset == "test":
                        ...
                        # [Added] For now, use everything and process it in run_gpt3.py to guarantee 5 positive samples.
                        instances = all_instances
                    
                    elif self.config.demo_num:
                        random.seed(42) # Added.
                        random.shuffle(all_instances)
                        instances = all_instances[:self.config.demo_num]
                        print("instance length", len(instances))

                    else:
                        instances = all_instances

                    if max_num_instances_per_task is not None and max_num_instances_per_task >= 0:
                        # Make sure to fix random seed.
                        random.seed(42) # Added.
